...
rule cat:
    ...
    shell:
        """
        for i in 1 2; do
            ls -v {params.path}/trimmed/*pair"$i".truncated.gz | xargs cat  > {params.outbase}$i.fq.gz
        done
        """
...
rule move_monos:
    ...
    output:
        R1_demulti=expand("{path}/Preprocessing/monos/{{sample}}.demultiplexed_R1.gz",  path=config["output_dir"]),
        R2_demulti=expand("{path}/Preprocessing/monos/{{sample}}.demultiplexed_R2.gz",  path=config["output_dir"])
    shell:
        """
        for i in 1 2; 
        do
            cp {params.inputdir}/{params.sample}.trimmed.pair"$i".truncated.gz {params.outputdir}/{params.sample}.demultiplexed_R"$i".gz
        done
        """
