...
rule temp_blastref:
    input:
        ref=expand("{tmp_dir}/Reference_creation/Renamed/{{sample}}.renamed.fa", tmp_dir=config["tmp_dir"]),
        blastresults=expand("{output_dir}/Blasting/results/{{sample}}.blastresults.tsv", output_dir=config["output_dir"])
    output:
...
rule blast_out:
    ...
    input:
        blastresults=expand("{output_dir}/Blasting/results/{sample}.blastresults.tsv", output_dir=config["output_dir"],sample=MONOS)
    output:
