...
rule mapping_Bowtie2:   
    ...
    conda: "../Envs/bowtie2.yaml"
    benchmark:"../Benchmarks/mapping_Bowtie2_{sample}.benchmark.tsv"
    output:
    ...
    shell:
        """
        bowtie2 -x ../Output/mapping/index -1 {input.r1} -2 {input.r2} -q --end-to-end --very-fast --threads {threads} -S {output.samOut} 2> {log}
...
#    conda: "../Envs/bwa.yaml"
#    benchmark:"../Benchmarks/mapping_BWA_index.benchmark.tsv"
...
#        r2=expand("{tmp}/Preprocessing/Sampleheaders/{{sample}}.2.fq.gz",  tmp=config["tmp_dir"])#, sample=SAMPLES)
#    conda: "../Envs/bwa.yaml"
#    benchmark:"../Benchmarks/mapping_BWA_{sample}.benchmark.tsv"
...
#    shell:
#        """
#        bwa mem -t {threads} -c 10 -R '@RG\\tID:{params.sample}\\tSM:{params.sample}' ../Output/mapping/index {input.r1} {input.r2} > {output.samOut} 2> {log}
#        samtools view -b -o {output.bamOut} {output.samOut}
#        """ #c 10
...
#    benchmark: "../Benchmarks/mapping_star_index.benchmark.tsv"
#    shell:
#        """
#        echo "Commencing STAR mapping" >> time.txt
#        date +%s%N >> time.txt
#        STAR --genomeSAindexNbases 10 --runThreadN {threads} --runMode genomeGenerate --genomeDir ../Output/mapping/index --genomeFastaFiles {input.refBlasted} --outTmpDir {params.tempMaps} --limitGenomeGenerateRAM 1600000000000 2> {log}
#        """
